...
#SBATCH --job-name=run_eval_metrics_full_perfect
...
scontrol update JobId=$SLURM_JOB_ID JobName=run_compute_metrics_full_perfect_${MODEL}

MODEL_DIR="${PROJECT_ROOT}/results/${MODEL}"
L96_DIR="${PROJECT_ROOT}/results/ensemble_l96"
OUT_DIR="${PROJECT_ROOT}/notebooks/evaluation/output/ensemble_evaluation/full_perfect"

N_INIT_STATES=300
N_ENS_MEMBERS=20
N_MODELS=20

conda run -n uncertainty python -u eval_metrics_full_perfect_driver.py \
  --model_dir "${MODEL_DIR}" \
  --l96_dir "${L96_DIR}" \
  --out_dir "${OUT_DIR}" \
