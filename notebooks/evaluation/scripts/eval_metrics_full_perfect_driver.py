...
from evaluation.scripts.eval_metrics_full_perfect import (  # noqa: E402
    compute_metrics_full_perfect_streamed,  # noqa: E402
)
...
flags.DEFINE_integer("n_models", 20, "N_models")
...
def main(argv):
    ...
    L = FLAGS.n_models
    if "baseline_det" in model_dir.name:
        logging.warning(
            "Baseline deterministic model detected. Overriding n_models to 1."
        )
        L = 1

    logging.info(
        "Starting metrics computation for model %s in full/perfect setting with N=%d, M=%d, L=%d.",
        model_dir.name,
        N,
        M,
        L,
    ...
    df, t = compute_metrics_full_perfect_streamed(model_dir, l96_dir, N, M, L)

    logging.info("Saving results to %s", out_dir)
    pickle_path = out_dir / f"{model_dir.name}_metrics_full_perfect.pkl"
    df.to_pickle(pickle_path)

    time_out = out_dir / f"{model_dir.name}_metrics_full_perfect_time.npy"
    np.save(time_out, t)
